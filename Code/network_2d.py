...
def pltcolor(lst):
    ...
    return cols
n=400
trainData,Z1_nonoise,Z2_nonoise  = genData(nSamples=n,frac=0.2)
X=trainData[:,0]
X=np.reshape(X,(-1,n)).T
Y=X.T
ix = np.random.choice(trainData.shape[0],int(trainData.shape[0]*0.01),replace=False)
...
ax2 = fig.add_subplot(111, projection='3d')
cols = pltcolor(testData[:,-1])
ax2.scatter(testData[:,0],testData[:,1],testData[:,2],marker='.',c=cols,s=5)
ax2.scatter(testData[:,0],testData[:,1],predData,marker='.',s=6,label=r'\textbf{Pred Data}')
...
ax2.set_ylabel(r'\textbf{Z}', fontsize=11)
fig.suptitle(r'\textbf{Network Prediction (2D Case)}', fontsize=11)
ax2.view_init(15,330)

ax2.figure.savefig(plot_path + "/modelmixed2_1.pdf", bbox_inches='tight')
