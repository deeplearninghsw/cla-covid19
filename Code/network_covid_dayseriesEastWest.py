...
def main():
    ...
    target = dfE[['DailyCases','west(0)/east(1)']]
    ...
    testtarget=dfW[['DailyCases','west(0)/east(1)']]
    ...
    model.compile(optimizer=opt,loss='logcosh',metrics=['logcosh'])
    history = model.fit(x=features,y=target[:,:1],batch_size=100 ,epochs=30)#,validation_split=0.1)
    ...
    model_json = model.to_json()
    with open("Plots/East.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("Plots/East.h5")
    ...
    for i in range(0,dfW.shape[0]/lastday):
        ...
        ax.legend()
        filename=plot_path + dfW['Landkreis'].iloc[i*lastday]+'.png'        
